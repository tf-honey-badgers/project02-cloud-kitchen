...
public class PaymentServiceImpl implements PaymentService {
	...
	public HttpEntity<MultiValueMap<String, String>> kakaopay(OrderVOExtend vo) {
		 ...
		 System.out.println(vo.getOrderDetails()[0].getMenuName()+" 외 "+(vo.getOrderDetails().length-1)+"개 ");
		    params.add("cid", "TC0ONETIME");
		    params.add("partner_order_id", vo.getId());
		    params.add("partner_user_id", vo.getCustId());
		    params.add("item_name", vo.getOrderDetails()[0].getMenuName()+" 외 "+(vo.getOrderDetails().length-1)+"개 ");
		    params.add("quantity", ""+vo.getOrderDetails().length);
		    params.add("total_amount", ""+vo.getPayAmt());
		    params.add("tax_free_amount", "0");
		    params.add("approval_url", "http://localhost:3001/customer/order/payment/kakaoPay/success");
		    params.add("cancel_url", "http://localhost:3001/customer/order/payment//kakaoPay/cancel");
		    params.add("fail_url", "http://localhost:3001/customer/order/payment/kakaoPay/fail");
