...
public class CartController {
	...
	public ResponseEntity<List<CartVOExtend>> deleteCart(@RequestBody Map<String, Object> req) {
		...
		if (returnVal != null) {
			log.info("Finished deleting from cart!!!!!");
		} else {
			log.info("Failed to delete from cart.");
		}
 ...
 * ResponseEntity<List<CartVOExtend>> addCart(@RequestBody CartVOExtend cart) {
 * List<CartVOExtend> returnVal = null; String url =
 * RestDomain.restDomain+"/cart/";
 * 
 * restTemplate.postForEntity(url, cart, String.class); // 카트에 메뉴 추가하기 url +=
 * cart.getCustId(); ResponseEntity<List> readMenuFromCart =
 * restTemplate.getForEntity(url, java.util.List.class); // 특정 회원ID를 가진 카트 항목들
 * 읽어오기 returnVal = readMenuFromCart.getBody();
 * 
 * if(returnVal != null) {
 * log.info("Finished adding selected menu to cart!!!!!"); } else {
 * log.info("Failed to add selected menu to cart."); }
 * 
 * return new ResponseEntity<>(returnVal, HttpStatus.OK); }
 */